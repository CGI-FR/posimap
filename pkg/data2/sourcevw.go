...
// SourceVariableWidth implements the Source interface for variable-width encodings.
type SourceVariableWidth struct {
	reader  *bufio.Reader
	encoder *encoding.Encoder
	decoder *encoding.Decoder
	working []byte // working buffer for decoding/encoding operations
}

func NewSourceVariableWidth(reader io.Reader, encoding encoding.Encoding) *SourceVariableWidth {
	if encoding == nil {
		encoding = unicode.UTF8
	}

	return &SourceVariableWidth{
...
func (s *SourceVariableWidth) ReadRunes(length int) ([]rune, error) {
